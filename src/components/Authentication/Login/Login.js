...
class Login extends React.Component {
  ...
  login(event) {
    ...
    axios
      .post("https://falconavl.com/gmc/api/Auth/login", {
        userName: email,
        password: password
      })
      .then(data => {
        ...
        this.props.onLogin({ IsAuthenticated: true, userInfo: data.data.token });
        ...
        <Container>
          <Row className="justify-content-center no-gutters vertical-align">
            <Col
              lg={4}
              md={6}
              className="login-fancy-bg bg"
              style={{
                backgroundImage: "url(assets/images/login-inner-bg.jpg)"
              }}
            >
